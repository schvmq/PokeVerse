...
export function MyLab({ capturedPokemon, onPokemonClick, onRelease, onUpdate }: MyLabProps) {
  ...
  const filteredPokemon = capturedPokemon.filter(c =>
    ...
    c.pokemon.id.toString().includes(searchTerm)
  );

  const startEdit = (captured: CapturedPokemon) => {
    setEditingId(captured.id);
    setEditNickname(captured.nickname || '');
    setEditNotes(captured.fieldNotes || '');
  };

  const saveEdit = (captureId: string) => {
    onUpdate(captureId, {
      nickname: editNickname || undefined,
      fieldNotes: editNotes || undefined,
    });
    setEditingId(null);
  };
        ...
        {filteredPokemon.length === 0 ? (
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            {filteredPokemon.map((captured, index) => {
              const isEditing = editingId === captured.id;
              const imageUrl = captured.pokemon.sprites.other.home?.front_default ||
                             captured.pokemon.sprites.other['official-artwork']?.front_default ||
                             captured.pokemon.sprites.front_default;
              ...
              return (
                ...
                >
                  <div className="flex flex-col sm:flex-row">
                    {/* Image Section */}
                    <div
                      onClick={() => onPokemonClick(captured.pokemon)}
                    ...
                    <div className="flex-1 p-6">
                      <div className="flex items-start justify-between mb-4">
                        <div className="flex-1">
                          <div className="text-slate-500 text-sm mb-1">
                            #{captured.pokemon.id.toString().padStart(3, '0')}
                          </div>
                          {isEditing ? (
                            <input
                              type="text"
                              ...
                              placeholder={captured.pokemon.name}
                            ...
                            <h3 className="text-2xl text-white capitalize mb-1">
                              {captured.nickname || captured.pokemon.name}
                            </h3>
                          )}
                        ...
                        {!isEditing && (
                          <div className="flex gap-2">
                            ...
                            <button
                              onClick={() => setReleaseConfirm(captured.id)}
                      ...
                      <div className="flex gap-2 mb-4">
                        {captured.pokemon.types.map(({ type }) => (
                          <span
                            key={type.name}
                      ...
                      <div className="mb-4">
                        ...
                        {isEditing ? (
                          ...
                          <p className="text-slate-300 text-sm italic">
                            {captured.fieldNotes || 'No field notes yet. Click edit to add observations.'}
                          </p>
                        )}
                      </div>
                      ...
                      <div className="flex items-center justify-between text-sm">
                        ...
                        <span className="text-slate-500">
                          {new Date(captured.capturedAt).toLocaleDateString()}
                        </span>
                      </div>
                  ...
                  {releaseConfirm === captured.id && (
                    ...
                    >
                      <p className="text-red-300 text-sm mb-3">
                        Release {captured.nickname || captured.pokemon.name} back into the wild?
                      </p>
                      <div className="flex gap-2">
                        <button
